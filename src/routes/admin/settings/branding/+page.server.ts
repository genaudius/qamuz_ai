...
export const load: PageServerLoad = async () => {
  try {
    ...
    const sidebarIconDarkFile = await getCurrentBrandingFile('sidebar-icon-dark');
    const sidebarIconLightFile = await getCurrentBrandingFile('sidebar-icon-light');
    ...
    return {
      settings: {
        ...
        currentSidebarIconDark: sidebarIconDarkFile?.url || null,
        currentSidebarIconLight: sidebarIconLightFile?.url || null,
    ...
    return {
      settings: {
        ...
        currentFavicon: null,
        currentSidebarIconDark: null,
        currentSidebarIconLight: null,
        logoWidth: "170",
        logoHeight: "27",
      },
      isDemoMode: isDemoModeEnabled()
...
export const actions: Actions = {
  update: async ({ request }) => {
    ...
    const sidebarIconDark = data.get('sidebarIconDark') as File
    const sidebarIconLight = data.get('sidebarIconLight') as File
    ...
    try {
      ...
      let faviconUrl = null;
      let sidebarIconDarkUrl = null;
      let sidebarIconLightUrl = null;
      ...
      // Handle dark sidebar icon upload
      if (sidebarIconDark && sidebarIconDark.size > 0) {
        try {
          // Delete existing dark sidebar icon first
          await deleteBrandingFile('sidebar-icon-dark');

          // Upload new dark sidebar icon
          const uploadedFile = await uploadBrandingFile(sidebarIconDark, 'sidebar-icon-dark');
          sidebarIconDarkUrl = uploadedFile.url;

          console.log('Dark sidebar icon uploaded successfully:', uploadedFile.filename);
        } catch (uploadError: any) {
          return fail(400, {
            error: uploadError.message || 'Failed to upload dark sidebar icon',
            logoWidth,
            logoHeight
          });
        }
      ...
      // Handle light sidebar icon upload
      if (sidebarIconLight && sidebarIconLight.size > 0) {
        try {
          // Delete existing light sidebar icon first
          await deleteBrandingFile('sidebar-icon-light');

          // Upload new light sidebar icon
          const uploadedFile = await uploadBrandingFile(sidebarIconLight, 'sidebar-icon-light');
          sidebarIconLightUrl = uploadedFile.url;

          console.log('Light sidebar icon uploaded successfully:', uploadedFile.filename);
        } catch (uploadError: any) {
          return fail(400, {
            error: uploadError.message || 'Failed to upload light sidebar icon',
            logoWidth,
            logoHeight
          });
        }
