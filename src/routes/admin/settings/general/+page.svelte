<script lang="ts">
  ...
  const systemPromptCharCount = $derived(openrouterSystemPrompt.length);
  const systemPromptRemaining = $derived(4000 - systemPromptCharCount);
  ...
  // State sync effect
  $effect(() => {
    siteName = form?.siteName || data?.settings?.siteName || "";
    siteTitle = form?.siteTitle || data?.settings?.siteTitle || "";
    siteDescription = form?.siteDescription || data?.settings?.siteDescription || "";
    selectedDefaultLanguage =
      form?.defaultLanguage || data?.settings?.defaultLanguage || "en";
    selectedDefaultTheme = form?.defaultTheme || data?.settings?.defaultTheme || "dark";
    selectedDefaultPage = form?.defaultPage || data?.settings?.defaultPage || "landing";
    publicOrigin = form?.publicOrigin || data?.settings?.publicOrigin || "";
  ...
  >
    ...
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description
          >Set default language, theme, and landing page for new users.
          Individual users can still override some of these preferences.</Card.Description
        >
      </Card.Header>
      <Card.Content class="space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div class="space-y-2">
            ...
            <Select.Root
              type="single"
              name="defaultPage"
              bind:value={selectedDefaultPage}
              disabled={data.isDemoMode}
            >
              <Select.Trigger id="defaultPage">
                {pageLabel}
              </Select.Trigger>
              <Select.Content>
            ...
            <p class="text-xs text-muted-foreground">
              The page users see when visiting the root URL
            </p>
          </div>
        </div>
      </Card.Content>
    ...
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description>
          Configure global system prompt for OpenRouter models (Claude, GPT,
          Gemini, etc.)
        </Card.Description>
      </Card.Header>
      <Card.Content class="space-y-4">
        <div class="space-y-2">
          <Textarea
            ...
            bind:value={openrouterSystemPrompt}
            disabled={data.isDemoMode}
            rows={6}
            maxlength={4000}
          />
          <div class="flex justify-between text-xs text-muted-foreground">
            <p>
              This prompt will be prepended to all OpenRouter model
              conversations
            </p>
            <p class={systemPromptRemaining < 500 ? "text-amber-500" : ""}>
              {systemPromptCharCount}/4000 characters
            </p>
          </div>
        </div>
      </Card.Content>
