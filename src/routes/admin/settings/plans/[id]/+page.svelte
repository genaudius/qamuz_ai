<script lang="ts">
  ...
  $effect(() => {
    tier = form?.tier || data.plan.tier;
    currency = form?.currency || data.plan.currency;
    billingInterval = form?.billingInterval || data.plan.billingInterval;
  });
  ...
  const billingTriggerContent = $derived(
    billingOptions.find((b) => b.value === billingInterval)?.label ??
      "Select interval"
  );
  ...
  <div class="flex justify-between items-center">
    <div>
      ...
      <p class="text-muted-foreground">
        Modify the pricing plan details below.
      </p>
    </div>
    <Button
      variant="outline"
      ...
      >
        ...
        <div class="space-y-4">
          ...
          <p class="text-sm text-muted-foreground">
            Leave empty for unlimited usage. Add 0 to exclude usage.
          </p>
            ...
            <div class="space-y-2">
              ...
              <Input
                ...
                value={form?.imageGenerationLimit ||
                  (data.plan.imageGenerationLimit ?? "")}
              />
            </div>
            ...
            <div class="space-y-2">
              ...
              <Input
                ...
                value={form?.videoGenerationLimit ||
                  (data.plan.videoGenerationLimit ?? "")}
              />
            </div>
