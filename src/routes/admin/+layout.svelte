<script lang="ts">
  ...
  import { LoaderIcon, MenuIcon } from "$lib/icons/index.js";
  ...
  const isNavigatingWithinAdmin = $derived.by(() => {
    ...
    const fromSettings = fromPath.startsWith("/admin/settings");
    const toSettings = toPath.startsWith("/admin/settings");

    return !(fromSettings && toSettings);
  });
...
{#snippet adminPageTransitionLoader()}
  <div
...
<div class="min-h-screen bg-background">
  ...
  <main class="container mx-auto px-6 py-8">
    {#if isNavigatingWithinAdmin}
      {@render adminPageTransitionLoader()}
    {:else}
      {@render children()}
    {/if}
  </main>
