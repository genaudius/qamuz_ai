<script lang="ts">
  ...
  const hasCredentialProvider = $derived(
    (data.authProviders || []).some(
      (provider) => provider.provider === "credential" || provider.type === "credentials"
    )
  );
  const socialAuthProviders = $derived(
    (data.authProviders || []).filter(
      (provider) => provider.type === "oauth" && provider.provider !== "credential"
    )
  );
  ...
  // Credential accounts require current password; OAuth-only accounts can set one.
</script>
    ...
    <Card.Content class="space-y-4">
      ...
      <div class="space-y-3">
        <div class="space-y-2">
          ...
          {#if (data.user as any).emailVerifiedAt}
            <p class="text-xs text-muted-foreground">
              {m["account.verified_on"]()} {new Date(
                (data.user as any).emailVerifiedAt
              ).toLocaleDateString()}
            </p>
          {/if}
        </div>
        ...
        {:else}
          <!-- Change Password Button -->
            ...
            >
              ...
              {hasCredentialProvider ? m["account.change_password"]() : "Set Password"}
            </Button>
          {#if data.isDemoMode}
            <p class="text-xs text-muted-foreground">
