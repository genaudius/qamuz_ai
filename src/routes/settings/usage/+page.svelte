<script lang="ts">
  ...
  const currentPeriod = $derived(data.currentPeriod);
  const usageSummary = $derived(data.usageSummary);
  const planTier = $derived(data.planTier);
  const modelUsage = $derived((data.modelUsageStatistics || []) as ModelUsageStatistic[]);

  // Convert usage summary to stats format
  const usageStats = $derived({
    textMessages: {
    ...
    audioGenerations: {
      used: usageSummary.audio.used,
      limit: usageSummary.audio.limit ?? -1,
      unit: m["usage.audio"]?.() ?? "audio",
    },
  });
          ...
          <div class="flex justify-between items-center">
            <span class="text-sm font-medium">
              {key === "textMessages" ? m["usage.text_messages"]() :
               ...
               key === "videoGenerations" ? m["usage.video_generations"]() :
               key === "audioGenerations" ? (m["usage.audio_generations"]?.() ?? "Audio Generations") : key}
            </span>
