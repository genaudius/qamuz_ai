...
export const POST: RequestHandler = async ({ request, locals, cookies }) => {
	...
	const credentialPasswordHash = credentialAccount?.password ?? null;
	...
	try {
		...
		// Clean up authentication tables (these should cascade, but let's be explicit)
		await db
			.delete(betterAuthSessions)
			.where(eq(betterAuthSessions.userId, session.user.id));
