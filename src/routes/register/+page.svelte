<script lang="ts">
  ...
  let clientError = $state(
    page.url.searchParams.get("error") || page.url.searchParams.get("oauthError")
      ...
      await authClient.signIn.social(
        {
          provider: "google",
          callbackURL: "/newchat",
          errorCallbackURL: "/register?oauthError=1",
          newUserCallbackURL: "/newchat",
        },
        {
          onError() {
...
<div class="min-h-screen flex items-center justify-center p-6 bg-muted/20">
  <Card.Root class="w-full max-w-md">
    ...
    <Card.Content class="space-y-4">
      ...
      >
        ...
        <p class="text-xs text-muted-foreground text-center">
          ...
          <a class="underline underline-offset-2" href={resolve("/terms")} target="_blank"
            >Terms of Service</a
          >.
        </p>
    ...
    <Card.Footer class="pt-2 text-center">
      <p class="text-sm text-muted-foreground">
        Already have an account?
        <a href={resolve("/login")} class="text-primary hover:underline cursor-pointer"
          >Log in</a
        >
      </p>
    </Card.Footer>
