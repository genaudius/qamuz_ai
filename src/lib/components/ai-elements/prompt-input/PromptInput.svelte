<script lang="ts">
	...
	interface Props {
		...
		onContextReady?: (context: AttachmentsContext) => void;
	...
	$effect(() => {
		attachmentsContext.setOptions(accept, multiple);
	});
	...
	// Convert File to data URL using FileReader (more reliable than fetching blob URLs)
	...
	let handleSubmit = async (event: SubmitEvent) => {
		...
		// Convert files to data URLs using stored File objects (more reliable than fetching blob URLs)
		let filesPromises = attachmentsContext.files.map(async ({ id, file, ...item }) => {
			if (file) {
				return {
					...item,
	...
	// Notify parent that context is ready
	$effect(() => {
		onContextReady?.(attachmentsContext);
	});
</script>
