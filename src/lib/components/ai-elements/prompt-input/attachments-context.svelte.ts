...
export interface FileWithId extends FileUIPart {
	id: string;
	file: File;  // Store the original File object for reliable data URL conversion
}
...
export class AttachmentsContext {
	...
	setOptions = (accept?: string, multiple?: boolean) => {
		this.accept = accept;
	...
	matchesAccept = (file: File): boolean => {
		...
		// Parse comma-separated accept patterns
		const patterns = this.accept.split(",").map((p) => p.trim().toLowerCase());

		for (const pattern of patterns) {
			// Handle wildcard MIME types (e.g., "image/*", "text/*")
			if (pattern.endsWith("/*")) {
			...
			// Handle specific MIME types (e.g., "application/json")
			else if (pattern.includes("/")) {
				if (file.type.toLowerCase() === pattern) {
					return true;
				}
			}
			// Handle file extensions (e.g., ".txt", ".md")
	...
	add = (files: File[] | FileList) => {
		...
		for (let file of capped) {
			next.push({
				...
				type: "file",
				file: file,  // Store original File for reliable data URL conversion
...
export class PromptInputController {
	...
	configure = (initialInput = "", accept?: string, multiple?: boolean) => {
		this.textInput.value = initialInput;
		this.attachments.setOptions(accept, multiple);
	};
}
