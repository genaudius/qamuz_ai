...
// Extended config for R2StorageProvider that optionally includes publicUrl
// Main bucket uses this without publicUrl, branding bucket uses it with publicUrl
...
class StorageService {
	...
	private async ensureInitialized() {
		...
		// Always update configVersion to track current state (must be outside if block)
	...
	private async getStorageConfig(): Promise<StorageConfig> {
		...
		// Note: Config version check is now done in ensureInitialized() before the early return,
		// so this method is only called when we actually need to (re)initialize the storage config.
	...
	generateFilePath(userId: string, fileType: 'images' | 'videos' | 'audio', filename: string, category: 'generated' | 'uploaded' = 'generated'): string {
	...
	async upload(file: StorageFile, userId: string, fileType: 'images' | 'videos' | 'audio', category: 'generated' | 'uploaded' = 'generated'): Promise<StorageResult> {
	...
	async uploadBrandingFile(file: StorageFile, category: string): Promise<StorageResult & { publicUrl?: string }> {
		...
		// Check if dedicated branding bucket is configured
		const brandingConfig = await this.getBrandingStorageConfig();

		if (!brandingConfig) {
			throw new Error(
				'Branding bucket is not configured. Please configure the Public Branding Bucket in Cloud Storage settings before uploading logos or favicons.'
			);
		}

		// Upload to dedicated branding bucket (no signed URLs)
		console.log(`Uploading branding file (${category}) to dedicated branding bucket: ${path}`);

		const brandingProvider = new R2StorageProvider({
			accountId: this.config!.r2Config!.accountId,
			accessKeyId: this.config!.r2Config!.accessKeyId,
			secretAccessKey: this.config!.r2Config!.secretAccessKey,
			bucketName: brandingConfig.bucketName,
			publicUrl: brandingConfig.publicUrl
		});

		const result = await brandingProvider.upload(file, path);

		// Return the public URL directly (no signing needed for branding bucket)
		const publicUrl = `${brandingConfig.publicUrl.replace(/\/$/, '')}/${path}`;
	...
	/**
	 * Get branding bucket configuration if available
	 * Returns null if branding bucket is not configured
	 */
	private async getBrandingStorageConfig(): Promise<{ bucketName: string; publicUrl: string } | null> {
		// Only applicable when main storage is R2
		if (this.config?.type !== 'r2' || !this.config.r2Config) {
			return null;
		}
		...
		try {
			...
			// Both bucket name and public URL are required
			if (bucketName && publicUrl) {
				return { bucketName, publicUrl };
			}
		} catch (error) {
			console.warn('Failed to get branding bucket config:', error);
		}
	...
	async migrateFile(localPath: string, userId: string, fileType: 'images' | 'videos' | 'audio', filename: string, mimeType: string): Promise<StorageResult | null> {
		await this.ensureInitialized();
