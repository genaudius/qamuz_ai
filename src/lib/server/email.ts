...
/**
 * Load an email template bundled at build time
...
function loadEmailTemplate(templateName: string): string {
  try {
    const templateMap: Record<string, string> = {
      'welcome-verify-email': welcomeVerifyEmailTemplate,
      'reset-password': resetPasswordEmailTemplate
    }

    const templateContent = templateMap[templateName]

    if (!templateContent) {
      throw new Error(`Template ${templateName} is not bundled in the build output`)
    }
...
class EmailService {
  ...
  constructor() {
    ...
    void this.reinitializeTransporter()
  }
  ...
  private async getSmtpConfig() {
    ...
    const hasRequiredConfig = !!(smtpConfig.host && smtpConfig.user && smtpConfig.pass)
    if (hasRequiredConfig) {
      // Cache only valid configuration. Avoid caching empty/invalid config on transient errors.
  ...
  private async initializeTransporter() {
    try {
      ...
      if (!smtpConfig.host || !smtpConfig.user || !smtpConfig.pass) {
        console.warn(`[Email Service] SMTP configuration not found from ${smtpConfig.source}. Email service will be disabled.`)
        return
      }
  ...
  private async reinitializeTransporter(): Promise<void> {
  ...
  async testConnection(): Promise<boolean> {
    const ready = await this.ensureTransporterReady()
