...
class EmailVerificationService {
  ...
  async createVerificationToken(userEmail: string): Promise<VerificationTokenResult> {
    ...
    try {
      ...
      await db.insert(betterAuthVerifications).values({
  ...
  async verifyToken(token: string): Promise<VerificationResult> {
    try {
      ...
      const verificationToken = await db
        ...
        .where(eq(betterAuthVerifications.value, token))
        .limit(1)
      ...
      if (userData.emailVerified || userData.emailVerifiedBool) {
  ...
  private async deleteToken(token: string): Promise<void> {
    try {
      await db
        .delete(betterAuthVerifications)
        .where(eq(betterAuthVerifications.value, token))
    } catch (error) {
  ...
  private async cleanupUserTokens(userEmail: string): Promise<void> {
    try {
      await db
        .delete(betterAuthVerifications)
        .where(eq(betterAuthVerifications.identifier, userEmail))
    } catch (error) {
  ...
  async cleanupExpiredTokens(): Promise<void> {
    try {
      ...
      await db
        .delete(betterAuthVerifications)
        .where(lt(betterAuthVerifications.expiresAt, now))
  ...
  async isEmailVerified(userEmail: string): Promise<boolean> {
    try {
      ...
      return (
        user.length > 0 &&
        (user[0].emailVerified !== null || user[0].emailVerifiedBool === true)
      )
    } catch (error) {
